...
createCopyTasks('locales', {
  source: './app/locales/',
  destinations: commonPlatforms.map(platform => `./cordova/www/locales`),
})
createCopyTasks('images', {
  source: './app/images/',
  destinations: commonPlatforms.map(platform => `./cordova/www/images`),
})
createCopyTasks('contractImages', {
  source: './node_modules/eth-contract-metadata/images/',
  destinations: commonPlatforms.map(platform => `./cordova/www/images/contract`),
})
createCopyTasks('fonts', {
  source: './app/fonts/',
  destinations: commonPlatforms.map(platform => `./cordova/www/fonts`),
})
createCopyTasks('reload', {
  devOnly: true,
  source: './app/scripts/',
  pattern: '/chromereload.js',
  destinations: commonPlatforms.map(platform => `./cordova/www/`),
})
...
createCopyTasks('manifest', {
  source: './app/',
  pattern: '/*.json',
  destinations: browserPlatforms.map(platform => `./cordova/www/`),
})
...
gulp.task('copy',
  gulp.series(
    ...
    // ,'manifest:production', //GZB MODIFY
    // 'manifest:chrome',
    // 'manifest:opera'
  )
)
...
function createTasksForBuildJsMascara({ taskPrefix, devMode, bundleTaskOpts = {} }) {
  ...
  // 修改打包到 ./cordova/www
  const destinations = ['./cordova/www/']
  bundleTaskOpts = Object.assign({
    buildSourceMaps: true,
    sourceMapDir: './',
    minifyBuild: !devMode,
...
function bundleTask(opts) {
  ...
  function performBundle(){
    ...
    // GZB MODIFY
    var isBackground = 'background.js' === opts.filename;
    if (isBackground){
