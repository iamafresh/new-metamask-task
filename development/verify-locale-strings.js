...
function verifyLocale({ localeMeta }) {
	...
	try {
		const localeFilePath = path.join(process.cwd(), 'app', 'locales', localeCode, 'messages.json')
	...
	try {
		const englishFilePath = path.join(process.cwd(), 'app', 'locales', 'en', 'messages.json')
